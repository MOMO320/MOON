...
{
	m_Map2->init();
	//m_bazookaFlower1->init(WINSIZEX/2 +400 , WINSIZEY/2);
	m_bazookaFlower2->init( 400 , WINSIZEY/2-50);
	PLAYER.init();
	//PLAYER.m_player.m_img = IMAGEMANAGER->findImage("dungeonPlayer");
...
{
	m_Map2->update();
	PLAYER.update(m_Map2);
	//m_bazookaFlower1->update(PLAYER.m_player._rc, m_Map2, -300 , 70,700, 200);
	m_bazookaFlower2->update(PLAYER.m_player._rc, m_Map2, 10 ,350,800 ,700);
}
