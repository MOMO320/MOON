...
void mapTool::mapToolMenuRender()
{
	...
	case SAVEMENU:
		// 세이브 로드 버튼 랜더
		printText(CAMERAMANAGER->getCameraDC(), tileTypeName[m_mapToolBookCount].c_str(), "나눔스퀘어_ac ExtraBold", WINSIZEX - 270, 135, 50, RGB(0, 0, 0), true, RGB(255, 0, 255));
		...
		{
			printText(CAMERAMANAGER->getCameraDC(), vc_tileFileName.at(i).c_str(), "나눔스퀘어_ac ", m_saveFile[i].rc.left+30, m_saveFile[i].rc.top+5 , 20, RGB(0, 0, 0), true, RGB(255, 0, 255));
			printText(CAMERAMANAGER->getCameraDC(), vc_tileFileName.at(i).c_str(), "나눔스퀘어_ac ", m_saveFile[i].rc.left+30, m_saveFile[i].rc.top + 30, 20, RGB(0, 0, 0), true, RGB(255, 0, 255));
		}
